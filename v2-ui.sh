...
install() {
    bash <(curl -Ls https://github.com/moyu1232/v2-ui/raw/master/v2-ui.sh)
    if [[ $? == 0 ]]; then
        if [[ $# == 0 ]]; then
            start
        else
...
uninstall() {
    ...
    echo -e "Github issues: ${green}https://github.com/moyu1232/v2-ui/issues${plain}"
...
install_bbr() {
    bash <(curl -L -s https://github.com/moyu1232/v2-ui/raw/master/bbr.sh)
    if [[ $? == 0 ]]; then
        echo ""
...
update_shell() {
    wget -O /usr/bin/v2-ui -N --no-check-certificate https://github.com/moyu1232/v2-ui/raw/master/v2-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
