...
install_bbr() {
    bash <(curl -L -s https://github.com/AstralHope/v2-ui/raw/master/bbr.sh)
    if [[ $? == 0 ]]; then
        echo ""
...
update_shell() {
    wget -O /usr/bin/v2-ui -N --no-check-certificate https://github.com/AstralHope/v2-ui/raw/master/v2-ui.sh
    if [[ $? != 0 ]]; then
        echo ""
